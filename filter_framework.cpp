...
int FilterFramework::Execute(int argc, char **argv){
...
bool FilterFramework::ProcessArgs(int argc, char **argv){
	if (argc < 2){
	...
	for(int i = 0; i < filters.size(); i++){
		if(flag.compare(filters[i]->GetFlag()) == 0){
			filter_index = i;
			break;
		}
	}
...
void FilterFramework::SetImageToFilters(){
...
void FilterFramework::ProcessInteraction(){
	std::cout << "Interactive mode" << std::endl;

	// set applicable filter filters that are shown on screen
	std::vector<Filter*> shown_filters;
	for(int i = 0; i < filters.size(); i++){
		if (filters[i]->IsApplicable()){
			shown_filters.push_back(filters[i]);
		}
	}
...
void FilterFramework::ExecuteFilter(Filter *filter){
...
inline void FilterFramework::ShowType(cv::Mat image){
#if SHOW_INPUT_TYPE
